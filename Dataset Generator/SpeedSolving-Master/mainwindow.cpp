...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    QObject::connect(ui->pushButton_ExportHistory, &QPushButton::clicked, this, &MainWindow::on_pushButton_ExportHistory_clicked);
    ...
    int Index = 0;
    foreach (QString filename, langfiles)
        if (CheckXMLLanguageFile(filename))
        {
...
void MainWindow::DisableWhileSearching()
{
    ...
    ui->comboBox_scrambleAmount->setDisabled(true);
    ...
    ui->pushButton_ExportHistory->setDisabled(true);
}
...
void MainWindow::EnableAfterSearch()
{
    ...
    ui->comboBox_scrambleAmount->setEnabled(true);
    ...
    ui->pushButton_ExportHistory->setEnabled(true);
}
...
void MainWindow::on_pushButton_Credits_clicked()
{
    ...
    if (file.open(QIODevice::ReadOnly | QIODevice::Text))
    {
        ...
        credits_text = CurrentLang["NoCredits"];
...
void MainWindow::on_pushButton_License_clicked()
{
    ...
    if (file.open(QIODevice::ReadOnly | QIODevice::Text))
    {
        ...
        license_text = CurrentLang["NoLicense"];
...
bool MainWindow::LoadXMLDefaultLanguage()
{
    ...
    if (lang_xml.LoadFile("default.xml") == XML_SUCCESS) // Read existing file
    {
        ...
        if (pRoot != nullptr)
        {
            ...
            if (Lang.compare("default") != 0)
                return false; // No default language
...
bool MainWindow::LoadXMLCurrentLanguage(const QString &lang)
{
    ...
    if (lang_xml.LoadFile(lang.toStdString().c_str()) == XML_SUCCESS) // Read existing file
    {
        ...
        {
            if (!CurrentLang.contains(i.key()))
                CurrentLang.insert(i.key(), i.value());
            ++i;
        }
...
{
    if (Lang.contains("MainWindowTitle"))
        setWindowTitle(Lang["MainWindowTitle"]);
    else
        setWindowTitle(DefaultLang["MainWindowTitle"]);
    ...
    while (i != Lang.constEnd())
    {
        ...
        { // In this application, fixed texts are only in push buttons, labels, check boxes and radio buttons
            if (QPushButton *pb = qobject_cast<QPushButton *>(widget))
                pb->setText(i.value());
            else if (QLabel *lb = qobject_cast<QLabel *>(widget))
                lb->setText(i.value());
    ...
    if (lang_xml.LoadFile(lang.toStdString().c_str()) == XML_SUCCESS) // Read existing file
    {
        ...
        if (pRoot != nullptr)
        {
            ...
            if (s_Language == nullptr)
                return false;
            std::string Lang = s_Language;
            if (Lang.empty() || lang.contains("default"))
                return false; // No language
            return true;
        }
    }
...
void MainWindow::on_lineEdit_Scramble_textChanged(const QString &scrInput)
{
    if (scrInput.isEmpty()) // No scramble
    {
        ...
        ui->pushButton_ClearScramble->setDisabled(true);
        // Do NOT disable Start Search button, allow solvers to run without scramble
    ...
    else
    {
        ...
        else
        {
            ...
            if (Score <= 0.0f)
                ui->label_Eval->setText(CurrentLang["NoEval"]);
            else if (Score < 6.221367f)
                ui->label_Eval->setText(CurrentLang["EvalVeryHard"]);
            else if (Score < 6.804842f)
                ui->label_Eval->setText(CurrentLang["EvalHard"]);
            else if (Score < 7.311247f)
                ui->label_Eval->setText(CurrentLang["EvalMediumHard"]);
            else if (Score < 8.007008f)
                ui->label_Eval->setText(CurrentLang["EvalMedium"]);
            else if (Score < 8.523252f)
                ui->label_Eval->setText(CurrentLang["EvalMediumEasy"]);
            else if (Score < 9.060186f)
                ui->label_Eval->setText(CurrentLang["EvalEasy"]);
            else
                ui->label_Eval->setText(CurrentLang["EvalVeryEasy"]);
        }
    }
}
...
{
    if (cores == 0)
        ui->statusBar->showMessage(CurrentLang["AllAvaliable"]);
    else if (cores == 1)
        ui->statusBar->showMessage(CurrentLang["SingleCoreWillBeUsedInTheSearch"]);
    else
        ui->statusBar->showMessage(CurrentLang["CoresWillBeUsedInTheSearch"] + QString::number(cores));
}
...
{
    if (ui->comboBox_History->count() == 0)
        return;
...
void MainWindow::AddToHistory(const QString &HistoryId)
{
    ...
    if (Solve.isEmpty() || HistoryId.isEmpty())
        return;
...
void MainWindow::on_pushButton_ExportHistory_clicked()
{
    if (History.empty())
    {
        ui->statusBar->showMessage("No history to export.");
        return;
    }

    QString fileName = QFileDialog::getSaveFileName(this, tr("Export history"), "", tr("JSON (*.json)"));
    if (fileName.isEmpty())
        return;
    ...
    {
        ui->statusBar->showMessage("Unable to open file for writing.");
        return;
    }
    ...
    {
        QJsonObject obj;
        obj["id"] = ui->comboBox_History->itemText(i);
        obj["report"] = History[i];
        arr.append(obj);
    }
    QJsonDocument doc(arr);
    file.write(doc.toJson(QJsonDocument::Indented));
    file.close();
    ui->statusBar->showMessage("History exported as JSON.");
}
...
void MainWindow::on_comboBox_Method_currentIndexChanged(const QString &arg1)
{
    ...
    case 1: // CFOP
        ...
        ui->comboBox_Option->addItem(CurrentLang["None"]);
        break;
    case 2: // Roux
        ...
        ui->comboBox_Option->addItem(CurrentLang["OneLookL6E"]);
        break;
    case 3: // Petrus
    ...
    case 5: // CEOR
        ...
        ui->comboBox_Option->addItem(CurrentLang["NoCPSkip"]);
        break;
    case 6: // Mehta
    ...
    case 8: // LEOR
        ...
        ui->comboBox_Option->addItem(CurrentLang["COLL+EPLL"]);
        break;
    default: // LBL
...
void MainWindow::on_pushButton_StartSearch_clicked()
{
    ...
    int scrambleAmount = getScrambleAmount();
    QString allResults;
    QStringList resultsList;
    for (int i = 0; i < scrambleAmount; ++i)
    {
        Algorithm Scramble(ui->spinBox_ScrambleLength->value());
        QString scrambleStr = QString::fromStdString(Scramble.ToString());
        ui->lineEdit_Scramble->setText(scrambleStr);
        // Call the appropriate search method based on selected method
        switch (ui->comboBox_Method->currentIndex())
        {
        case 0:
            sLBL(Scramble.ToString());
            break;
        case 1:
            sCFOP(Scramble.ToString());
            break;
        case 2:
            sRoux(Scramble.ToString());
            break;
        case 3:
            sPetrus(Scramble.ToString());
            break;
        case 4:
            sZZ(Scramble.ToString());
            break;
        case 5:
            sCEOR(Scramble.ToString());
            break;
        case 6:
            sMehta(Scramble.ToString());
            break;
        case 7:
            sNautilus(Scramble.ToString());
            break;
        case 8:
            sLEOR(Scramble.ToString());
            break;
...
void MainWindow::printMethod_Solves(const QString Scramble, const QString Result)
{
    ...
    if (ui->comboBox_Option->count() > 1)
        HtmlReport += ", " + OptionText + " " + Option;
    HtmlReport += "</h2>";
...
void MainWindow::printMethod_Best(const QString Result)
{
    ...
    {
    case 1:
        HtmlReport += CurrentLang["CFOPBest"];
        break;
    case 2:
        HtmlReport += CurrentLang["RouxBest"];
        break;
    case 3:
        HtmlReport += CurrentLang["PetrusBest"];
        break;
    case 4:
        HtmlReport += CurrentLang["ZZBest"];
        break;
    case 5:
        HtmlReport += CurrentLang["CEORBest"];
        break;
    case 6:
        HtmlReport += CurrentLang["MehtaBest"];
        break;
    case 7:
        HtmlReport += CurrentLang["NautilusBest"];
        break;
    case 8:
        HtmlReport += CurrentLang["LEORBest"];
        break;
    default:
        break;
    }
    ...
    for (auto &Line : Lines)
    {
        ...
        if (Parts.size() == 2)
            HtmlReport += "<code style=\"color: rgb(127, 0, 0);\">" + Parts[0] + "</code><span> // " + Parts[1] + "</span><br>";
        else if (Parts.size() == 1)
            HtmlReport += "<span>" + Parts[0] + "</span><br>";
    }
}
...
void MainWindow::printMethod_Time(const QString Result)
{
    ...
    case 6:
        ...
        break;
    case 7:
        AddToHistory(CurrentLang["NautilusSolve"]);
        break;
    case 8:
        AddToHistory(CurrentLang["LEORSolve"]);
        break;
...
int MainWindow::getScrambleAmount() const
{
    if (ui && ui->comboBox_scrambleAmount)
        return ui->comboBox_scrambleAmount->currentText().toInt();
    return 1;
}